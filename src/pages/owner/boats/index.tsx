...
export default function OwnerBoats() {
  ...
  const fetchBoats = async () => {
    ...
    try {
      ...
      const mapped: Boat[] = items.map((b: any) => ({
        ...
        cabins: [],
        services: [],
        images: b.thumbnailUrl
          ? [{ id: '1', imageUrl: b.thumbnailUrl, caption: '' }]
          : [],
      }));
      setBoats(mapped);
    } catch {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast.error('Không thể xóa thuyền');
    }
  };
  ...
  const KPI_STATS = [
    ...
    },
    {
      ...
      color: '#10B981',
      bg: 'rgba(16,185,129,0.12)',
      change: `${stats.total ? Math.round((stats.running / stats.total) * 100) : 0}% tổng`,
      up: true,
    },
    {
      label: 'Nhàn rỗi',
